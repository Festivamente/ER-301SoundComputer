...
namespace od
{
    ...
    // Explicit float casts: clang treats double->float narrowing in an
    // initializer list as an error (gcc only warns).
    const float arcOffsets[] = {0, (float)sin(M_PI * 0.167), (float)sin(M_PI * 0.333), 1,
                                (float)sin(M_PI * 0.333), (float)sin(M_PI * 0.167), 0};
