...
namespace
{
  static void appendPathComponent(std::ostringstream &oss,
...
namespace od
{
  ...
  {
    std::string path = mRootPath;
    for (std::string &dir : mStack)
    {
      std::ostringstream oss;
      appendPathComponent(oss, path, dir);
      path = oss.str();
    }
    mPath = path;
    mPathDirty = false;
  ...
  const std::string &FileBrowser::getSelectedFullPath()
  {
    ...
    std::ostringstream oss;
    appendPathComponent(oss, mPath, getSelectedName());
    mFullPath = oss.str();
