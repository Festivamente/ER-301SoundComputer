...
namespace od
{
	...
	{
		float *end = dst + NUM_OUTPUT_CHANNELS * FRAMELENGTH;
		dst += channel;
	...
	void OutputTask::process(float *inputs, float *outputs)
	{
		...
		{
			zeroOutput(outputs, 3);
			memset(mMonitor4.buffer(), 0, FRAMELENGTH * sizeof(float));
		}
	}

	void OutputTask::prepareForShutdown()
	{
		// Release references to UnitChain outputs before TaskScheduler begins
		// deleting tasks. The scheduler still owns every chain at this point,
		// so disconnecting cannot trigger re-entrant chain destruction.
		mOut1.disconnect();
		mOut2.disconnect();
		mOut3.disconnect();
		mOut4.disconnect();

		mMonitor1.disconnect();
		mMonitor2.disconnect();
		mMonitor3.disconnect();
		mMonitor4.disconnect();
	}
