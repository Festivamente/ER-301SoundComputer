...
namespace od
{
    ...
    void ConnectionQueue::prepareForShutdown()
    {
        mMutex.enter();

        // Pending queue entries each own one temporary reference to every
        // non-null pointer. Do not execute graph mutations during shutdown;
        // simply cancel the entries and return those references.
        for (Item &item : mPending)
        {
            ...
            if (item.object)
            {
                item.object->setScheduledFlag(false);
                item.object->release();
            }
        }
        mPending.clear();
