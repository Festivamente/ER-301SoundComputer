...
namespace od
{
  ...
  void UnitChain::prepareForShutdown()
  {
    // Teardown must be two-phase. First sever all graph connections while the
    // scheduler still retains this chain and all other tasks. Actual Unit
    // release remains in the destructor after every task has been prepared.
    mMutex.enter();
    disconnectInternals();
    disconnectOutputs();
    clearInputs();
