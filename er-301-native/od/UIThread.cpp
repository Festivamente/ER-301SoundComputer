...
namespace od
{
  ...
  void UIThread::init()
  {
    if (local)
    {
      logWarn("UIThread::init called while already initialized.");
      return;
    }
  ...
  void UIThread::stopRealtimeJobs()
  {
    if (!local)
    {
      return;
    }
    local->realtimeJobQueue.stop();
    local->realtimeJobQueue.join();
  }
  ...
  void UIThread::shutdown()
  {
    ...
    // Stop asynchronous work before releasing graphics and audio-backed
    // objects it might still reference.
    stopRealtimeJobs();

    setMainGraphicContext(0);
    setSubGraphicContext(0);
    setMainOverlay(0);
    setSubOverlay(0);
