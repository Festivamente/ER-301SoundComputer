...
// Embedding hosts can ask the Lua application thread to serialize the
// complete ER-301 state. Hardware builds keep the constant/API as harmless
// no-ops so the shared SWIG interface remains identical.
#define EVENT_HOST_SAVE EVENT(EVENT_SYSTEM, 3)
...
#define EVENT_USB_REAR_CARD_UNMOUNT EVENT(EVENT_USB, 7)
  ...
  // Synchronously request a host snapshot from the Lua application thread.
  // Hardware builds provide harmless no-op implementations.
