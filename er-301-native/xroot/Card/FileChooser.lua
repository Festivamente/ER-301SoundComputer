...
function FileChooser:init(opts)
  ...
  self.goal = opts.goal
  self.computerBrowsing = app.EMULATION and
                             (opts.goal == "load file" or
                              opts.goal == "load files")
  -- Every desktop/emulation load chooser gets the native host browser. The
  -- ER-301 browser remains available through Front (and Enable Multi where
  -- applicable), but users should not have to crawl the host filesystem with
  -- the hardware UI when the OS already provides a file chooser.
  self.hostBrowse = self.computerBrowsing
  self.hostBrowseTimer = nil
  self.computerRoot = "/"
  self.volumesRoot = "/Volumes"

  self.multi = false
  self.staged = opts.staged or 0
  self.extension = opts.ext

  -- Convert the ER-301 FileBrowser glob(s) into osdialog's extension-filter
  -- syntax. This keeps native Browse faithful to each original chooser: unit
  -- presets show .unit, chain presets .chain, samples .wav/.multi, etc.
  local hostExtensions = {}
  local function addHostPattern(glob)
    if type(glob) ~= "string" then
      return
    end
    local ext = glob:match("^%*%.(.+)$")
    if ext and ext ~= "*" and ext ~= "" then
      hostExtensions[#hostExtensions + 1] = ext
    end
  end
  if type(opts.pattern) == "table" then
    for _, glob in ipairs(opts.pattern) do
      addHostPattern(glob)
    end
  else
    addHostPattern(opts.pattern)
  end
  if #hostExtensions == 0 and type(self.extension) == "string" then
    local ext = self.extension:gsub("^%.", "")
    if ext ~= "" then
      hostExtensions[1] = ext
    end
  end
  self.hostFilter = (#hostExtensions > 0) and
                      ("ER-301 files:" .. table.concat(hostExtensions, ",")) or
                      ""
  ...
  elseif opts.goal == "load file" then
    ...
    self.subGraphic:addChild(label)
    -- sub buttons
    label = app.TextPanel(self.hostBrowse and "Browse" or
                            (self.computerBrowsing and "Computer" or ""), 1,
                          0.5, app.GRID5_LINE3)
    self.subGraphic:addChild(label)
    self.S1 = label
    label = app.TextPanel(self.computerBrowsing and "Front" or "", 2,
                          0.5, app.GRID5_LINE3)
    self.subGraphic:addChild(label)
    self.S2 = label
    label = app.TextPanel(self.computerBrowsing and "Volumes" or "", 3,
                          0.5, app.GRID5_LINE3)
    self.subGraphic:addChild(label)
    self.S3 = label
...
function FileChooser:navigateTo(path)
  ...
  elseif Utils.startsWith(path, app.roots.rear) then
    root = app.roots.rear
  elseif self.computerBrowsing and Utils.startsWith(path, Path.sep) then
    root = self.computerRoot
  else
    root = app.roots.x
  end
  ...
  if root == self.computerRoot then
    -- A POSIX root splits to empty components. Push every non-empty path
    -- component so /Users/name/Music remains fully navigable up to /.
    for _, dir in ipairs(dirs) do
      if dir ~= "" then
        browser:pushDirectory(dir)
      end
    end
  else
...
function FileChooser:stopHostBrowseTimer()
  if self.hostBrowseTimer then
    Timer.cancel(self.hostBrowseTimer)
    self.hostBrowseTimer = nil
  end
end

function FileChooser:browseHost()
  if not self.hostBrowse or self.multi then
    return
  end

  self:stopPreview()
  self:stopHostBrowseTimer()

  -- Start in the last external directory when history provides one. For the
  -- virtual ER-301 card/runtime roots, let the OS choose its normal default.
  local initialPath = self.browser:getSelectedPath() or ""
  if Utils.startsWith(initialPath, app.roots.front) or
      Utils.startsWith(initialPath, app.roots.rear) or
      Utils.startsWith(initialPath, app.roots.x) then
    initialPath = ""
  end

  if not app.HostFileDialog_request(initialPath, self.hostFilter or "") then
    return
  end

  self.hostBrowseTimer = Timer.every(0.1, function()
    local status = app.HostFileDialog_status()
    if status == 1 then
      local fullpath = app.HostFileDialog_takeResult()
      self.hostBrowseTimer = nil
    ...
    elseif status == 2 then
      -- Native dialog cancellation leaves the ER-301 chooser open.
      app.HostFileDialog_takeResult()
      self.hostBrowseTimer = nil
      return false
    end
  end)
...
function FileChooser:browseComputer()
  if self.computerBrowsing then
    self:stopPreview()
    self:navigateTo(self.computerRoot)
  end
end

function FileChooser:browseFront()
  if self.computerBrowsing then
    self:stopPreview()
    self:navigateTo(app.roots.front)
  end
end

function FileChooser:browseVolumes()
  if self.computerBrowsing then
    self:stopPreview()
    if Path.exists(self.volumesRoot) then
      self:navigateTo(self.volumesRoot)
    else
      self:navigateTo(self.computerRoot)
    end
  end
...
function FileChooser:toggleMultiSelection()
  if self.multi then
    ...
    self.S1:setText(self.hostBrowse and "Browse" or
                    (self.computerBrowsing and "Computer" or ""))
    self.S2:setText(self.computerBrowsing and "Front" or "")
...
function FileChooser:onHide()
  self:stopPreview()
  self:stopHostBrowseTimer()
  if self.hostBrowse then
    app.HostFileDialog_reset()
  end
end
...
function FileChooser:subReleased(i, shifted)
  ...
  elseif self.goal == "load files" then
    if self.multi then
      ...
      if i == 1 and self.computerBrowsing then
        if self.hostBrowse then
          self:browseHost()
        else
          self:browseComputer()
        end
      elseif i == 2 and self.computerBrowsing then
        self:browseFront()
      elseif i == 3 then
        self:toggleMultiSelection()
      end
