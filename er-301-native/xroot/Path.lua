...
function Path.isAbsolute(path)
  if app.EMULATION and Utils.startsWith(path, Path.sep) then
    return true
  end
...
function Path.recursiveDelete(path)
  ...
  -- dir() intentionally hides dot files from the ER-301 user interface.
  -- ER-301 Sound Computer's native package loader stores prepared images in the hidden
  -- .er-301soundcomputer-native folder, so cleanup must enumerate every entry instead.
  local iterator = dirAll or dir
  local success = true
  for fname in iterator(path) do
    local path2 = Path.join(path, fname)
    if app.isDirectory(path2) then
      if not Path.recursiveDelete(path2) then
        app.logError("Failed to delete %s", path2)
        success = false
      end
    else
      if not app.deleteFile(path2) then
        app.logError("Failed to delete %s", path2)
        success = false
      end
    end
  ...
  if not app.deleteDirectory(path) then
    app.logError("Failed to delete %s", path)
    success = false
  end

  return success and not app.pathExists(path)
end
