...
function Interface:doUninstallPackage()
  ...
  local package = Manager.getPackage(id)
  if package then
    local success, msg = Manager.uninstall(package)
    if success then
      self:refresh()
    elseif msg then
      local Message = require "Message"
      Message.Main("Uninstall failed.", msg):show()
    end
  end
end
...
function Interface:doDeletePackage()
  ...
  if package then
    ...
    local task = function(ans)
      if ans then
        local success, msg = Manager.delete(package)
        if success then
          self:refresh()
        elseif msg then
          local Message = require "Message"
          Message.Main("Delete failed.", msg):show()
        end
      end
    end
...
function Interface:enterReleased()
  ...
  local success, msg
  if package.installed then
    success, msg = Manager.uninstall(package)
  else
    success, msg = Manager.install(package)
  end
  if success then
    self:refresh()
  elseif msg then
    local Message = require "Message"
    Message.Main("Package operation failed.", msg):show()
  end
  return true
end
