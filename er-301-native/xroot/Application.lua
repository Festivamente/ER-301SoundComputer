...
Busy.start("ER-301 Sound Computer (%s)", app.FIRMWARE_VERSION)
...
-- Rack copies the module's patch-storage snapshot here before the ER-301
-- application starts. Keeping this outside the versioned hardware restore
-- path prevents Rack patch recall from overwriting normal ER-301 quicksaves.
local function getRackRestorePath()
  local Path = require "Path"
  return Path.join(app.roots.rear, "rack-restore.save")
end

local function restore()
  local Path = require "Path"
  local rackPath = getRackRestorePath()
  local fromRack = Path.exists(rackPath)
  local path = fromRack and rackPath or getRestorePath()
  if Path.exists(path) then
    ...
    if preset:read(path) then
      -- Rack patch recall must reproduce the complete saved instrument,
      -- independent of the user's optional quicksave restore preferences.
      restored = preset:apply(fromRack) ~= false
    else
...
local function saveRackPatchState()
  local success = false
  local ok, result = pcall(function()
    local Path = require "Path"
    local QuickSavePreset = require "Persist.QuickSavePreset"
    local path = Path.join(app.roots.rear, "rack-current.save")
    local preset = QuickSavePreset()
    app.logInfo("Saving Rack patch state to %s", path)
    preset:populate()
    return preset:write(path)
  end)
  if ok then
    success = result == true
  else
    app.logError("Rack patch state save failed: %s", tostring(result))
  end
  app.HostState_finishSave(success)
end
...
local function loop()
  ...
  local eventHostSave = app.EVENT_HOST_SAVE
  local quit = false
  while not quit do
    waitForEvent()
    startEventTimer()
    ...
    while true do
      ...
      elseif e == eventDisplayReady then
        onDisplayReady()
      elseif eventHostSave and e == eventHostSave then
        saveRackPatchState()
  ...
  local SamplePool = package.loaded["Sample.Pool"]
  if SamplePool and SamplePool.shutdown then
    SamplePool.shutdown()
  end

  -- Release visible contexts before destroying the channel groups they refer
  -- to. The original hardware relied on process exit; the Rack host must tear
  -- the graph down explicitly so a module can be removed safely.
  setVisibleContext(nil)
  setActiveMode(nil)

  local Channels = package.loaded["Channels"]
  if Channels and Channels.shutdown then
    Channels.shutdown()
  end

  if Card and Card.shutdown then
    Card.shutdown()
  end
