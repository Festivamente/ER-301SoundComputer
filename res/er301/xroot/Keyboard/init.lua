...
function Keyboard:init(msg, initial, extended, history)
  ...
  -- In the VCV Rack host, mirror the hardware keyboard with a normal desktop
  -- text field. This is only an input shortcut: the ER-301's own validator,
  -- history, and done signal below remain authoritative.
  self.hostTextDialogTimer = nil
  self.hostTextDialogStartTimer = Timer.after(0.05, function()
    self.hostTextDialogStartTimer = nil
    if not app.HostTextDialog_request or
        not app.HostTextDialog_request(msg or "Enter text", initial or "",
                                       extended == true) then
      return
    end

    self.hostTextDialogTimer = Timer.every(0.05, function()
      local status = app.HostTextDialog_status()
      if status == 1 then
        local text = app.HostTextDialog_takeResult() or ""
        self.hostTextDialogTimer = nil
        self.textEditor:setText(text)
        -- Commit through enterReleased() so all original ER-301 validation
        -- and naming/file rules are applied exactly as on the hardware.
        self:enterReleased()
        return false
      elseif status == 2 then
        -- Cancelling the Rack popup leaves the original ER-301 keyboard open.
        app.HostTextDialog_takeResult()
        self.hostTextDialogTimer = nil
        return false
      end
    end)
...
function Keyboard:stopHostTextDialogTimers()
  if self.hostTextDialogStartTimer then
    Timer.cancel(self.hostTextDialogStartTimer)
    self.hostTextDialogStartTimer = nil
  end
  if self.hostTextDialogTimer then
    Timer.cancel(self.hostTextDialogTimer)
    self.hostTextDialogTimer = nil
  end
end
