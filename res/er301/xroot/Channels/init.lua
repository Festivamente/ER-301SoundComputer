...
local function shutdown()
  ...
  -- Channel groups normally live until the hardware powers down. A Rack
  -- module can be deleted and recreated, so destroy every unique root group
  -- while Lua, AudioThread, and the scheduler are still alive.
  local seen = {}
  if groups then
    for _, group in ipairs(groups) do
      if group and not seen[group] then
        ...
        if not ok then
          app.logError("Channels.shutdown: %s", err)
        end
      end
    end
  end
