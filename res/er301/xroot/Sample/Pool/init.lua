...
local function serializeSample(sample)
  ...
  local t = {}
  -- ER-301 Sound Computer can load a sample from anywhere on the host computer, not only
  -- from the virtual front/rear card roots. External single-file samples
  -- are still path-backed and must keep that path in Rack patch state.
  if sample:isFileBased() or sample:isShared() or
      sample.opts.type == "single" then
    t.path = sample.path
  end
  if sample:isFileBased() then
...
local function shutdown()
  ...
  -- Keep saver, loader, and their queue alive while the active file job is
  -- canceled and joined. This avoids Lua GC releasing a job object while
  -- its native worker thread is still using it during host teardown.
  jobQ:cancel(loader)
  jobQ:cancel(saver)
  jobQ:stop()
  jobQ:join()
